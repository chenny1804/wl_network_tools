...
def addDevice(dev_id,dev_name,playurl,priority,ssid,passwd):
	if dev_id in localDev:
		dev_entity=StaDev(dev_id,dev_name,playurl,priority,ssid,passwd)
		dev_entity.ConnectWifi()
		new_thread = threading.Thread(target=AnalysisThread,args=(dev_entity,))	
		new_thread.start()
		thread[dev_entity.GetId()] = 1
		StaDevArray.append(dev_entity)	
		return "add a devices - " + dev_id  
	else
		return "not found the devices = " + dev_id

def stopDevice(dev_id):
	for dev_entity in StaDevArray
		if dev_entity.GetId()==dev_id:
			thread[dev_entity.GetId()] = 0
			dev_entity.DisconnectWifi()
			return "stop a devices - " + dev_id
	
	return "the devices don't start - " dev_id
...
def initLocalDev():
	...
	for line in iter(f):
		if line.find(dev_id)!=-1:
			localDev.append(line.split()[0].strip())
	f.close()

		

	
def startDev(device_list,period,interval,mode):
	print "start Dev"
	#将控制端配置文件写入本地
...
#	server = SimpleXMLRPCServer.SimpleXMLRPCServer(('localhost', 28888))
	...
	server.register_function(addDevice)
	server.register_function(stopDevice)
